  ...
  return (
    <AlertDialogPortal>
      ...
      <AlertDialogPrimitive.Content
        ...
        className={cn(
          'bg-background data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 fixed top-[50%] left-[50%] z-50 grid w-full max-w-[calc(100%-2rem)] translate-x-[-50%] translate-y-[-50%] gap-6 rounded-3xl border border-white/10 p-6 shadow-xl duration-200 sm:max-w-lg md:p-8',
          className
        )}
        {...props}
      />
...
function AlertDialogAction({
  ...
  return (
    <AlertDialogPrimitive.Action
      className={cn(
        buttonVariants(),
        'rounded-3xl px-6 py-2.5 transition-all duration-300',
        className
      )}
      {...props}
    />
